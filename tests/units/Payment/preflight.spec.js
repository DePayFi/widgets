...
describe('Payment', () => {
  ...
  describe('preflight', () => {
    ...
    it('requires you to set a blockchain', async ()=> {
      await expect(
        ()=>DePayWidgets.Payment({ accept: [{ blockchain: undefined }], document })
    ...
    it('requires you to set a supported blockchain', async ()=> {
      await expect(
        ()=>DePayWidgets.Payment({ accept: [{ blockchain: 'notsupported' }], document })
      ).rejects.toEqual('DePayWidgets.Payment: You need to set a supported blockchain!')
    })

    it('requires you to set an amount', async ()=> {
      await expect(
        ()=>DePayWidgets.Payment({ accept: [{ blockchain: 'ethereum', amount: undefined }], document })
    ...
    it('requires you to set a token', async ()=> {
      await expect(
        ()=>DePayWidgets.Payment({ accept: [{ blockchain: 'ethereum', amount: '20', token: undefined }], document })
      ).rejects.toEqual('DePayWidgets.Payment: You need to set the token you want to receive as payment!')
    })

    it('requires you to set a token', async ()=> {
      await expect(
        ()=>DePayWidgets.Payment({ accept: [{ blockchain: 'ethereum', amount: '20', token: '0xa0bEd124a09ac2Bd941b10349d8d224fe3c955eb', receiver: undefined }], document })
