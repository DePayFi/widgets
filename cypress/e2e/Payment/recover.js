      ...
      cy.visit('cypress/test.html').then((contentWindow) => {
        cy.document().then((document)=>{
          ...
          cy.wait(2000).then(()=>{
            expect(!!websocketMessages.find((rawMessage)=>{
              ...
              return(
                ...
                message.identifier == JSON.stringify({ blockchain, sender: fromAddress, nonce: transactionNonce, channel: 'TransactionChannel' })
              )
            })).to.equal(true)
          ...
          cy.wait(1000).then(()=>{
            ...
            cy.get('.ReactShadowDOMOutsideContainer').shadow().contains('.Card', 'Transaction confirmed').invoke('attr', 'href').should('include', `https://etherscan.io/tx/${transactionId}`)
