...
class PaymentDialog extends React.Component {
    ...
    new DePay.ethers.Contract(this.props.selected.token.address, Erc20Abi, DePay.ethers.provider)
      ...
      .then(function(transaction){
        if(transaction) {
          ...
          transaction.wait(3).then(this.checkApproved.bind(this));
        } else {
          this.setState({ approving: false })
        }
      }.bind(this))

    this.approvalCheckInterval = setInterval(this.checkApproved.bind(this), 1000);
  }
  ...
  performPayment() {
    ...
    .catch(function(){
      Rollbar.error("performPayment catch", arguments);
      this.setState({ paying: false });
    }.bind(this))
    .then(function(transaction){
      if(transaction) {
        transaction.wait(3).then(function(){
          this.setState({
            paying: false,
            payed: true
          })
        ...
        Rollbar.error("performPayment then", arguments);
        this.setState({ paying: false })
      }
    }.bind(this));

    this.paymentCheckInterval = setInterval(this.checkApproved.bind(this), 1000);
  }
  ...
  renderApproveButton() {
    if(this.state.approving) {
      ...
      return(
        <button key='approve' className='CallToAction' onClick={this.approve.bind(this)} title='Click to approve that the selected token is allowed to be swapped for performing this payment. This approval is only required the first time you pay with the selected token.'>
          Approve
        </button>
      )
    }
